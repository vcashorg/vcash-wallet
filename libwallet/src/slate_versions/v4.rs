...
use crate::grin_core::core::{TokenInput, TokenOutput};
...
impl From<&TokenOutput> for TokenCommitsV4 {
	fn from(out: &TokenOutput) -> TokenCommitsV4 {
		TokenCommitsV4 {
			k: out.token_type(),
			f: out.features().into(),
			c: out.commitment(),
...
// This will need to be reworked once we no longer support input features with "commit only" inputs.
impl From<&TokenInput> for TokenCommitsV4 {
	fn from(input: &TokenInput) -> TokenCommitsV4 {
		TokenCommitsV4 {
			k: input.token_type(),
			f: input.features.into(),
			c: input.commitment(),
